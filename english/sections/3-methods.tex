...
This chapter outlines the methodology used to develop and evaluate the proposed video object detection models. We introduce the Recurrent Perceiver (RPerceiver) and its multi-modal variant (RPerceiverMM) architectures. We then describe the custom dataset generated for this work and the specific training procedures employed, including dropout and shuffling for model robustness. Finally, we detail the loss function used for optimization and the metrics selected for performance evaluation.
