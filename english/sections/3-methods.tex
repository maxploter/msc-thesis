...
% TODO: refine equations

\begin{equation}
    \text{ADE} = \frac{1}{M \cdot T} \sum_{i=1}^{M} \sum_{t=1}^{T} || \hat{p}_{i,t} - p_{i,t} ||_2
    \label{eq:ade}
\end{equation}

where, $N$ is fixed-size of potential center point predictions p, $T$ is the number of time steps in each sequence, $\hat{y}_{i,t}$ is the predicted center point for sequence $i$ at time step $t$, and $y_{i,t}$ is the corresponding ground truth center point.
...
\begin{equation}
    \text{FDE} = \frac{1}{M} \sum_{i=1}^{M} || f(p_{i,T}) - p_{i,T} ||_2
    \label{eq:fde}
\end{equation}

where, $ f(p_{i,T}) $ is matching function introduced \ref{Methods:LossFunction} takes predicted center point for ground truth center point $i$ at the final time step $T$, and $p_{i,T}$ is the ground truth center point at that same final time step.
